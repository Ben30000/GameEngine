...
//	private int depthPassMVPLocation;
	...
	public StageItem(double x, double y, double z, float[] vertices, float[] normalsSmooth, float[] normalsPlanar, int[] indicies, ArrayList<Wrap> materials, ArrayList<float[]> uvsMaterials, Background b, 
		...
		this.normalsPlanar = normalsPlanar;
		...
		// Indexed version, can only be SMOOTH
		float bitangentsSmooth[] = new float[vertices.length];
		float bitangentsPlanar[] = new float[vertices.length];
		...
		// Non indexed version, can be SMOOTH, PLANAR, or BOTH
		else {
			...
			if (normalsPlanar != null) {
				bitangentsPlanar = new float[vertices.length];
				for (int r = 0; r < bitangentsPlanar.length; r++) {			
					bitangentsPlanar[r] = 0.0f;
				}
			}
			...
			for (int r = 0; r < vertices.length/3; r = r + 3) {
		            ...
		            if (normalsPlanar != null) {
		            	this.tangentsPlanar[r]   = tangent.x;
		            	this.tangentsPlanar[r+1] = tangent.y;
		            	this.tangentsPlanar[r+2] = tangent.z;
		            	this.tangentsPlanar[r+3] = tangent.x;
		            	this.tangentsPlanar[r+4] = tangent.y;
		            	this.tangentsPlanar[r+5] = tangent.z;
		            	this.tangentsPlanar[r+6] = tangent.x;
		            	this.tangentsPlanar[r+7] = tangent.y;
		            	this.tangentsPlanar[r+8] = tangent.z;
		            	
		            	bitangentsPlanar[r]   = bitangent.x;
		            	bitangentsPlanar[r+1] = bitangent.y;
		            	bitangentsPlanar[r+2] = bitangent.z;
		            	bitangentsPlanar[r+3] = bitangent.x;
		            	bitangentsPlanar[r+4] = bitangent.y;
		            	bitangentsPlanar[r+5] = bitangent.z;
		            	bitangentsPlanar[r+6] = bitangent.x;
		            	bitangentsPlanar[r+7] = bitangent.y;
		            	bitangentsPlanar[r+8] = bitangent.z;
		            }
...
"layout (location=3) in vec3 nVecPlanar;"+
"layout (location=4) in vec3 tangentSmooth;"+
"layout (location=5) in vec3 tangentPlanar;"+
"layout (location=6) in vec3 bitangentSmooth;"+
"layout (location=7) in vec3 bitangentPlanar;"+
...
"vec2 parallaxMap(vec3 viewDirection) {"+
        ...
        "	vec2 deltaUV = (viewDirection.xy * pScale) / (viewDirection.z * numberLayers);"+
        "	vec2 currentUV = fTxt;"+
...
"    vec2 p = (vDirection.xy*pScale)/(vDirection.z) ;"+
  ...
  */
    ...
    " float wSDepth =  (21.0*pScale ) * depthFromTexture;"+
...
//	       		  " fragColor = vec4( exp(c*gl_FragCoord.z), 0.0, 0.0, 1.0);"+
	       ...
	       String shadowCubeFShaderCode =
		    ...
		    if (normalsPlanar != null) {
			 FloatBuffer normalsBuffer = null;
			    try {
		            normalsBuffer = MemoryUtil.memAllocFloat(normalsPlanar.length);
		            (normalsBuffer.put(normalsPlanar)).flip();
		            glBindVertexArray(vaoId);

		            vboIdPlanarN = glGenBuffers();
		            glBindBuffer(GL_ARRAY_BUFFER, vboIdPlanarN);
		            glBufferData(GL_ARRAY_BUFFER, normalsBuffer, GL_STATIC_DRAW);            
		            glVertexAttribPointer(this.numberVBOSlots, 3, GL_FLOAT, false, 0, 0);
		    ...
		    this.numberVBOSlots++;
				 ...
				 if (normalsPlanar != null) {
					 tangentsBuffer = null;
					    try {
				            tangentsBuffer = MemoryUtil.memAllocFloat(tangentsPlanar.length);
				            (tangentsBuffer.put(tangentsPlanar)).flip();
				            glBindVertexArray(vaoId);
				            vboIdTansPlanar = glGenBuffers();
				            glBindBuffer(GL_ARRAY_BUFFER, vboIdTansPlanar);
				            glBufferData(GL_ARRAY_BUFFER, tangentsBuffer, GL_STATIC_DRAW);            
				            glVertexAttribPointer(this.numberVBOSlots, 3, GL_FLOAT, false, 0, 0);
				            glBindVertexArray(0);
				            glBindBuffer(GL_ARRAY_BUFFER, 0);  
				 ...
				 if (normalsPlanar != null) {
					 tangentsBuffer = null;
					    try {
				            tangentsBuffer = MemoryUtil.memAllocFloat(bitangentsPlanar.length);
				            (tangentsBuffer.put(bitangentsPlanar)).flip();
				            glBindVertexArray(vaoId);
				            vboIdBitsPlanar = glGenBuffers();
				            glBindBuffer(GL_ARRAY_BUFFER, vboIdBitsPlanar);
				            glBufferData(GL_ARRAY_BUFFER, tangentsBuffer, GL_STATIC_DRAW);            
				            glVertexAttribPointer(this.numberVBOSlots, 3, GL_FLOAT, false, 0, 0);
				            glBindVertexArray(0);
				            glBindBuffer(GL_ARRAY_BUFFER, 0);  
	...
	// Water uniforms
		...
		Vector2f waterBody1S = new Vector2f(24.0f,0.808f);
		   ...
		   ////("For Stage Item, Program Id and Vao Id are " + programId + " and " + vaoId);
	        ...
	        glUseProgram(programId);
	        glBindVertexArray(vaoId);
	        System.out.println("Before anabling attribs");
	        for (int k = 0; k < this.numberVBOSlots; k++) {
	        	glEnableVertexAttribArray(k);
	        }
	        System.out.println("After anabling attribs");
...
////("Vao is " + vaoId + " and Vbo is " + vboId + " and programId is " + programId);
	 ...
	 //       System.out.println("Indicies length is " + indicies.length);
	      //glEnable(GL_CULL_FACE);
		    ...
		    if (this.indicies != null) {
		    	// INDEXED 
		    	System.out.println("About to draw, indexed");
		    	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,eboID);
		        glBufferData(GL_ELEMENT_ARRAY_BUFFER,indexBuffer,GL_STATIC_DRAW);
		    	glDrawElements(GL_TRIANGLES,indicies.length,GL_UNSIGNED_INT,0);
		    } else {
		    	// NON-INDEXED
		    	System.out.println("About to draw, non indexed");
		    	glDrawArrays(GL_TRIANGLES,0,(this.vertices.length)/3);
		    }
System.out.println("After draw call");
	        glCullFace(GL_BACK);
	       glDisable(GL_CULL_FACE);
